...
const createAgent = (req) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const agentIdNo = generateAgentId(body.agentAdhaar);

            const agentPhotoFile = req?.files?.agentPhoto?.[0]?.agentPhoto || null;
            const agentSignatureFile = req?.files?.agentSignature?.[0]?.agentSignature || null;
            let res = await agentModel.create({
                ...
                agentPassword: md5(body.agentPassword),
